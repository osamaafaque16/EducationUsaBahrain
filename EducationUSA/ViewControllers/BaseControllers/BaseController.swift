...
////
////        rightBarButton?.addBadge(text: "5", withOffset: CGPoint.zero, andColor: .red, andFilled: true, andFontSize: 15)
//
    ...
    func addBadge(badgeNumber:String){
        ...
        if badgeNumber == "0" {
            ...
            badgeCount.text = badgeNumber
        }
      


        let rightBarButton = UIButton(frame: CGRect(x: 0, y: 0, width: 20, height: 20))
        rightBarButton.setImage(UIImage(named: "notification"), for: .normal)
        rightBarButton.addTarget(self, action: #selector(BaseController.showNotificationController), for: .touchUpInside)
...
//           bagButton.setImage(UIImage(named: "notification")?.withRenderingMode(.alwaysTemplate), for: .normal)
