...
import WebKit
class StudentVisaController: BaseController , UITextViewDelegate{
    ...
    override func viewDidLoad() {
        ...
        let underLineAttribute = [ NSAttributedStringKey.font: UIFont.systemFont(ofSize: 19) ,NSAttributedStringKey.foregroundColor : Utility.UIColorFromRGB(rgbValue: 0xCD002E),NSAttributedStringKey.underlineStyle : NSUnderlineStyle.styleSingle.rawValue] as [NSAttributedStringKey : Any]
...
//        }
    ...
    func callDetailService(){
        ...
        let url = "\(BASE_URL)student-visa-detail/5"
        
        StudentVisaBase.getStudentVisaDetail(urlPath: url, parameter: parameter, vc: self) { (data) in
            ...
            self.eduData = data.result?.studentVisaDetail
            self.setData()
        }
...
extension StudentVisaController: UIWebViewDelegate{
...
extension StudentVisaController: WKNavigationDelegate{
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        if navigationAction.navigationType == .linkActivated{
            ...
            if url?.contains("https//") ?? false{
                let a = URL(string: (url?.replacingOccurrences(of: "https//", with: ""))!)
                UIApplication.shared.canOpenURL(a!)
